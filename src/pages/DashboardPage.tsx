...
export default function DashboardPage() {
  ...
  const handleResolve = async (id: string, status: 'DISMISSED' | 'ACTION_TAKEN') => {
  ...
  const { platformStats, moderationQueue, categoryHealth, platformLoadSeries, leaderboard } = data;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard label="Problems Solved" value={platformStats.totalSolved.toLocaleString()} icon={<CheckCircle2 size={18} />} accentClassName="text-primary-light" />
        <StatCard label="Verified Experts" value={platformStats.expertsVerified.toLocaleString()} icon={<ShieldCheck size={18} />} />
        <StatCard label="Success Rate" value={platformStats.successRate} icon={<TrendingUp size={18} />} accentClassName="text-success" />
        <StatCard label="Active Problems" value={String(platformStats.activeProblems)} icon={<Flame size={18} />} accentClassName="text-warning" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="rounded-xl2 border border-cardBorder bg-card p-5 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/moderation" className="text-xs font-medium text-primary-light">View all</Link>
          </div>
          <div className="space-y-3">
            {moderationQueue.length === 0 ? (
              ...
              moderationQueue.map((item) => (
                <div key={item.id} className="flex flex-col gap-2 rounded-lg border border-cardBorder bg-bgElevated p-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="secondary" onClick={() => handleResolve(item.id, 'DISMISSED')}>Dismiss</Button>
                    <Button size="sm" variant="danger" onClick={() => handleResolve(item.id, 'ACTION_TAKEN')}>Remove</Button>
                  </div>
                </div>
              ))
            )}
        ...
        <div className="rounded-xl2 border border-cardBorder bg-card p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/leaderboard" className="text-xs font-medium text-primary-light">View all</Link>
          </div>
          <div className="space-y-3">
            {leaderboard.map((u) => (
            ...
            {categoryHealth.map((c) => (
              <div key={c.id}>
                <div className="mb-1 flex items-center justify-between text-sm">
                  ...
                  <span className="text-textSecondary">{c.openCount} open · {c.solvedRate}% solved</span>
                </div>
                <div className="h-2 w-full overflow-hidden rounded-full bg-bgElevated">
                  <div className="h-full rounded-full" style={{ width: `${c.solvedRate}%`, backgroundColor: c.color }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={platformLoadSeries}>
                <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: '#5C6178', fontSize: 11 }} />
                <Tooltip contentStyle={{ background: '#161A26', border: '1px solid #22273A', borderRadius: 8 }} labelStyle={{ color: '#F4F5F8' }} />
